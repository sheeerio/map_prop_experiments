...
def init_layer(key, input_size, output_size, var, temp, l_type, max_units):
    ...
    _w_real = jrandom.uniform(subkey, shape=(input_size, output_size), minval=-lim, maxval=lim)
...
def init_network(key, state_n, action_n, hidden, var, temp, hidden_l_type, output_l_type, optimizer_factory):
...
def forward_network(key, network, state):
    ...
    (final_x, key), new_layers = lax.scan(layer_forward, (state, key), network["layers"])
...
if __name__ == "__main__":
    ...
    # Initialize network.
    network, optimizers, key = init_network(key, state_n, action_n, hidden, var, temp,
                                              hidden_l_type, output_l_type, optimizer_factory)
