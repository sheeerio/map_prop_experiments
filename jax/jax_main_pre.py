...
def main():
    ...
    parser.add_argument(
        "--key", type=int, default=0, help="PRNG Key for generating keys and subkey(s)."
    )
    # Parse all arguments
    ...
    for j in range(args.n_run):
        ...
        for i in range(args.max_eps // args.batch_size):
            ...
            map_grad_ascent(
                key, net=net, steps=args.map_grad_ascent_steps, update_size=update_size
            )
            ...
            if (i * args.batch_size) > print_count:
                running_avg = jnp.average(
                    jnp.array(eps_ret_hist[-print_every // args.batch_size :])
                )
