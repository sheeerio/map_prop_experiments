...
class EqPropLayer:
    ...
    inputs: Optional[
        jnp.ndarray
    ] = None  # latest inputs (e.g. shape (batch, input_size))
    pot: Optional[jnp.ndarray] = None  # potential (pre-activation)
    mean: Optional[
        jnp.ndarray
    ] = None  # post-activation mean (or softmax probabilities)
...
def init_eq_prop_layer(
    key, name, input_size, output_size, optimizer, var, temp, l_type
):
...
def update_layer(
    ...
    layer: EqPropLayer,
    update_size: float,
    next_layer: Optional[EqPropLayer] = None,
):
    if next_layer is None:
        if layer.l_type in LS_REAL:
            sigma = jnp.sqrt(1.0 / layer._inv_var)
            key, subkey = jrandom.split(key)
            new_values = layer.mean + sigma * jrandom.normal(
                subkey, shape=layer.pot.shape
            )
        else:
            key, subkey = jrandom.split(key)
        ...
        if layer.l_type in LS_REAL:
            ...
            if next_layer.l_type in LS_REAL:
                ...
                upper_term = (
                    upper_term
                    * ACT_D_F[next_layer.l_type](next_layer.pot)
                    * next_layer._inv_var
                )
                ...
                fb_w = jnp.transpose(next_layer._w)
                upper_pot = (
                    jnp.dot((next_layer.values - next_layer.mean), fb_w)
                    / next_layer.temp
                )
...
@jit
def record_trace_layer(
    layer: EqPropLayer, gate: Optional[jnp.ndarray] = None, lambda_: float = 0.0
):
    """
    ...
    if layer.inputs is None or layer.pot is None or layer.mean is None:
        raise ValueError(
            "Layer state is incomplete. Make sure to run sample/compute before trace."
        )
    if layer.l_type in LS_REAL:
        v_ch = (
            (layer.values - layer.mean)
            * ACT_D_F[layer.l_type](layer.pot)
            * layer._inv_var
        )
    else:
...
def learn_trace_layer(layer: EqPropLayer, reward: jnp.ndarray, lr: float = 0.01):
    ...
    delta_w = layer.optimizer.delta(
        grads=[w_update], name=layer.name + "_w", learning_rate=lr
    )[0]
    delta_b = layer.optimizer.delta(
        grads=[b_update], name=layer.name + "_b", learning_rate=lr
    )[0]
