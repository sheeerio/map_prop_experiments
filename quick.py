...
def sigmoid(x):
    lim = 20.0
    return jnp.where(
        x <= -lim, 0.0, jnp.where(x >= lim, 1.0, 1.0 / (1.0 + jnp.exp(-x)))
    )
...
def multinomial_rvs(key, n, p):
    ...
    def body(i, state):
        ...
        sample = jax.random.binomial(subkey, n=count, p=condp[..., idx]).astype(
            jnp.int32
        )
...
class complex_multiplexer_MDP(MDP):
    ...
    def reset(self, batch_size):
        ...
        self.x = jax.random.bernoulli(subkey, p=0.5, shape=(batch_size, x_size)).astype(
            jnp.int32
        )
        x_reshaped = self.x[:, : addr_size * action_size].reshape(
    ...
    def expected_reward(self, p):
        ...
        reward_f = reward_f.at[
            jnp.arange(self.x.shape[0]), y_zero.astype(jnp.int32)
        ].set(1)
...
def act_complex_multiplexer(y, actions, reward_zero):
    ...
    return jnp.where(
        reward_zero,
        corr,
        (corr > 0).astype(jnp.float32) - (corr <= 0).astype(jnp.float32),
    )
...
def eq_prop_record_trace(
    inputs, values, mean, pot, inv_var, temp, l_type, gate, lambda_, w_trace, b_trace
):
    if l_type in LS_REAL:
...
class eq_prop_layer:
    ...
    def compute_pot_mean(self, inputs):
        self.inputs = inputs
        self.pot, self.mean = eq_prop_compute_pot_mean(
            self.inputs, self._w, self._b, self.l_type, self.temp, self._inv_var
        )

    def sample(self, inputs):
        self.inputs, self.rng, self.pot, self.mean, self.values = eq_prop_sample(
            self.rng, inputs, self._w, self._b, self.l_type, self.temp, self._inv_var
        )
        return self.values
...
class Network:
    ...
    def map_grad_ascent(
        self, steps, state=None, gate=False, lambda_=0, update_size=0.01
    ):
...
def main():
    ...
    if args.env_name == "Multiplexer":
        env = complex_multiplexer_MDP(
            addr_size=5, action_size=1, zero=False, reward_zero=False
        )
        gate = False
    ...
    for j in range(args.n_run):
        ...
        for i in range(args.max_eps // args.batch_size):
            ...
            if (i * args.batch_size) > print_count:
                running_avg = np.average(
                    eps_ret_hist[-print_every // args.batch_size :]
                )
