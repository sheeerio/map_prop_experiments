...
def adam_update(params, grads, state: AdamState, meta: AdamParams):
    t = state.t + 1
    m = tree_util.tree_map(lambda m, g: meta.beta1*m + (1-meta.beta1)*g, state.m, grads)
    v = tree_util.tree_map(lambda v, g: meta.beta2*v + (1-meta.beta2)*(g**2), state.v, grads)
...
def relu_d(x): return jnp.where(x>0, 1, 0)
def sigmoid(x): return 1/(1 + jnp.exp(-jnp.clip(x, -20,20)))
def sigmoid_d(x): s=sigmoid(x); return s*(1-s)
def softplus(x): return jnp.log1p(jnp.exp(jnp.clip(x, -30,30)))
...
def init_layer(rng, in_dim, out_dim, var, temp, l_type, lr):
    ...
    zero = jnp.zeros((1,out_dim)); trace_w=jnp.zeros((1,in_dim,out_dim)); trace_b=jnp.zeros((1,out_dim))
...
def compute_pot_mean(params: LayerParams, inputs: jnp.ndarray, l_type: int, temp: float):
...
@jax.jit
def layer_sample(rng, meta: LayerMeta, inputs: jnp.ndarray) -> Tuple[jnp.ndarray, LayerMeta, jax.random.KeyArray]:
...
@jax.jit
def layer_map_ascent(meta: LayerMeta, inputs: jnp.ndarray, next_pot_mean_inv: Tuple[jnp.ndarray,jnp.ndarray], update_size: float) -> LayerMeta:
    # unpack
    ...
    if next_pot_mean_inv is None:
        ...
        if next_ltype in LS_REAL:
            upper = ((state.new_values - next_mean)* [relu_d, softplus, lambda x:1, sigmoid_d][next_ltype](next_pot)*next_inv_var) @ params.w.T
        else:
...
def net_map_ascent(params: NetParams, state, inputs, update_sizes: List[float]):
    ...
    for i, meta in enumerate(metas):
        ...
        meta = layer_map_ascent(meta, pots_means[i][1].shape and inputs if i==0 else metas[i-1].state.new_values, next_info, update_sizes[i])
        new_metas.append(meta)
...
def pure_complex_multiplexer(batch_size, addr_size=5, action_size=1):
    ...
    x = jax.random.bernoulli(key, 0.5, (batch_size, addr_size*action_size + 2**addr_size)).astype(jnp.int32)
    # compute y as before
    x_reshaped = x[:, :addr_size*action_size].reshape(batch_size, action_size, addr_size)
...
@jax.jit
def from_one_hot(p): return jnp.argmax(p, axis=-1)

def zero_to_neg(x): return (x>1e-8).astype(jnp.float32) - (x<=1e-8).astype(jnp.float32)
...
def main():
    ...
    temp   = args.temp
    l_types= [args.l_type]*len(hidden) + [4]  # e.g. same hidden type, discrete output
    ...
    for epoch in range(1):
        ...
        net_params, _ = adam_update(net_params, grads, net_opt_state, AdamParams(lr_meta,0.9,0.999,1e-9))
