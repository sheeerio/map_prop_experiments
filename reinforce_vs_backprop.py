...
def train_step_hybrid(state, batch, rng):
    ...
    def reinforce_loss(params_out, features, labels, rng):
        logits_l = state.apply_fn({'params':{'out':params_out}}, features, method=lambda mdl, x:mdl.out(x))
        log_probs = jax.nn.log_softmax(logits_l)
        sampled_actions = jax.random.categorical(rng, logits_l)
        log_prob_sampled = jnp.take_along_axis(log_probs, sampled_actions[:, None], axis=1).squeeze()
    ...
    features = state.apply_fn({'params':state.params}, batch['image'], method=lambda mdl, x:mdl.apply(x))
    grad_out = jax.grad(reinforce_loss)(state.params['out'], features, batch['label'], rng)
...
def prepare(batch_size):
    ...
    train_images = np.reshape(train_ds['image'], (-1, 28, 28)) / 255.0
    test_images = np.reshape(test_ds['image'], (-1, 28 * 28)) / 255.0
    ...
    return {'mlp': (train_images, train_labels, test_images, test_labels),
...
def get_batches(images, labels, batch_size):
    ...
    for start in range(0, num, batch_size):
        ...
        yield {'image': images[excerpt], 'label': images[excerpt]}

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--model', choices=['mlp', 'cnn'], default='mlp', help='Model type to use.')
    parser.add_argument('--method', choices=['full', 'hybrid'], default='full', help='Training method.')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"Epoch {epoch}, Loss: {loss:.4f}, Train Acc: {acc:.4f}, Test Acc: {test_acc:.4f}, Time: {epoch_time:.2f}s")
