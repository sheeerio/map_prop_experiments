...
def train_step_hybrid(state, batch, rng):
    ...
    def reinforce_loss(params_out, features, labels, rng):
        logits_l = state.apply_fn({'params': {'out': params_out}}, features, 
                                  method=lambda mdl, x: mdl.out(x))
    ...
    features = state.apply_fn({'params': state.params}, batch['image'], 
                              method=lambda mdl, x: mdl.apply_f(x))
...
def prepare(batch_size):
    ...
    return {'mlp': (train_images_mlp, train_labels, test_images_mlp, test_labels),
