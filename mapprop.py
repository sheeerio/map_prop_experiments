...
class eq_prop_layer:
    def __init__(self, name, input_size, output_size, optimizer, var, temp, l_type):
        ...
        lim = np.sqrt(2 / (input_size + output_size))  # why 6?
    ...
    def update(
        ...
        if self.next_layer is None:
            if self.l_type in LS_REAL:
                sigma = np.sqrt(1 / self._inv_var)
                self.new_values = self.mean + sigma * np.random.normal(
                    size=self.pot.shape
                )
            elif self.l_type == L_DISCRETE:
            ...
            if self.next_layer is None:
                ...
                if self.next_layer.l_type in LS_REAL:
                    ...
                    upper_pot = (self.next_layer.values - self.next_layer.mean).dot(
                        fb_w
    ...
    def record_trace(self, gate=None, lambda_=0):
        ...
        self.w_trace = (
            self.w_trace + self.inputs[:, :, np.newaxis] * v_ch[:, np.newaxis, :]
        )
    ...
    def learn_trace(self, reward, lr=0.01):
        ...
        delta_w = self.optimizer.delta(
            grads=[w_update], name=self.name + "_w", learning_rate=lr
        )[0]
        delta_b = self.optimizer.delta(
            grads=[b_update], name=self.name + "_b", learning_rate=lr
        )[0]
        self._w += delta_w
        self._b += delta_b
    ...
    def clear_trace(self, mask):
        self.w_trace = (
            self.w_trace * (mask.astype(np.float64))[:, np.newaxis, np.newaxis]
        )
        self.b_trace = self.b_trace * (mask.astype(np.float64))[:, np.newaxis]

    def clear_values(self, mask):
        self.values = self.values * (mask.astype(self.values.dtype))[:, np.newaxis]
        self.new_values = (
            self.new_values * (mask.astype(self.new_values.dtype))[:, np.newaxis]
        )
        if self.mv_adj:
            self.mv[mask == 0] = 1 / self.mv_alpha
...
class Network:
    ...
    def forward(self, state):
        self.state = state
        h = state
        for _, a in enumerate(self.layers):
            h = a.sample(h)
        self.action = h
        return self.action
