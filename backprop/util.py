...
def binary_lin_z(x):
    ...
    y = np.zeros_like(x)
    y[x < -c_2] = 0.0
    y[(-c_2 <= x) & (x < -c_1)] = ((p * x + c_2 * p) / (c_2 - c_1))[
        (-c_2 <= x) & (x < -c_1)
    ]
    y[(-c_1 <= x) & (x < c_1)] = ((0.5 - p) / c_1 * x + 0.5)[(-c_1 <= x) & (x < c_1)]
    y[(c_1 <= x) & (x < c_2)] = ((p * x + c_2 - c_1 - p * c_2) / (c_2 - c_1))[
        (c_1 <= x) & (x < c_2)
    ]
    y[x >= c_2] = 1.0
    return y
...
class xor_MDP(MDP):
    ...
    def act(self, actions):
        corr = (self.y[self.s][:, np.newaxis] == actions.astype(np.int32)).astype(
            np.float32
        )
...
class k_arm_binary_bandit_2(MDP):
    ...
    def act(self, actions):
        ...
        r = (action_sum < self.c) * (action_sum / self.c) + (action_sum >= self.c) * (
            1 - (action_sum - self.c) / self.c
        )
        r = np.clip(r, 0, 1)
        if not self.zero:
            r = r * 2 - 1
...
def real_to_bin(real, bin_min=0, bin_max=1, bin_num=10):
    bin_index = np.array((real - bin_min) / (bin_max - bin_min) * bin_num).astype(
        np.int
    )
    bin_index -= 1
...
def est_V(env, policy, T, bin_min, bin_max, bin_num, gamma):
    ...
    for t in range(T - 1, -1, -1):
        ...
        bin_state = tuple(
            real_to_bin(
                real=state[valid], bin_min=bin_min, bin_max=bin_max, bin_num=bin_num
            ).T
        )
...
def batch_autograd(v, inputs, batch_size, **args):
    ...
    for b in range(batch_size):
        ...
        for k in range(len(inputs)):
            grads[k][b] = g[k] if g[k] is not None else 0.0
    return grads
...
class State_to_spike_Fourier:
    def __init__(
        ...
        self.time = time
        self.out_shape = (
            ((self._c.shape[-1]) * (2 if double else 1) + (1 if basis else 0)) * rep,
            1,
        )
        self.basis = basis
        self.soft = soft
    ...
    def __call__(self, state, augment=None):
        ...
        if self.soft:
            bin_output = np.broadcast_to(
                ...
                (self.time, state.shape[0], self.out_shape[0] // self.rep, self.rep),
            )
            bin_output = bin_output.reshape(
                self.time, state.shape[0], self.out_shape[0], 1
            )
        else:
            bin_output = np.random.binomial(
                n=1,
                p=f_basis,
                size=(
                    self.time,
            ...
            bin_output = np.swapaxes(bin_output, 1, 2).reshape(
                self.time, state.shape[0], -1, 1
            )
