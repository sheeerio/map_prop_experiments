...
def adam_update(params, grads, opt_state, lr, beta1=0.9, beta2=0.999, epsilon=1e-09):
    t = opt_state["t"] + 1
    new_m = tree_util.tree_map(
        lambda m, g: beta1 * m + (1 - beta1) * g, opt_state["m"], grads
    )
    new_v = tree_util.tree_map(
        lambda v, g: beta2 * v + (1 - beta2) * jnp.square(g), opt_state["v"], grads
    )
    ...
    new_params = tree_util.tree_map(
        lambda p, m, v: p + lr * m / (jnp.sqrt(v) + epsilon), params, m_hat, v_hat
    )
...
def multinomial_rvs(key, n, p):
    ...
    def body_fn(carry, i):
        ...
        binsample = jax.random.binomial(subkey, n=count, p=condp[..., i]).astype(
            jnp.int32
        )
...
def linear_interpolat(start, end, end_t, cur_t):
    if isinstance(start, list):
        if isinstance(end_t, list):
            return [
                (e - s) * jnp.minimum(cur_t, d) / d + s
                for (s, e, d) in zip(start, end, end_t)
            ]
        else:
            return [
                (e - s) * jnp.minimum(cur_t, end_t) / end_t + s
...
ACT_F = {L_SOFTPLUS: softplus, L_RELU: relu, L_SIGMOID: sigmoid, L_LINEAR: lambda x: x}

ACT_D_F = {
    L_SOFTPLUS: sigmoid,  # derivative of softplus is sigmoid
    L_RELU: relu_d,
    L_SIGMOID: sigmoid_d,
    L_LINEAR: lambda x: 1,
}
...
class eq_prop_layer:
    def __init__(self, name, input_size, output_size, optimizer, var, temp, l_type):
        ...
        self._w = jax.random.uniform(
            jax.random.PRNGKey(random.randint(0, 2**31 - 1)),
        ...
        self.opt_state_w = {
            "m": jnp.zeros_like(self._w),
            "v": jnp.zeros_like(self._w),
            "t": 0,
        }
        self.opt_state_b = {
            "m": jnp.zeros_like(self._b),
            "v": jnp.zeros_like(self._b),
            "t": 0,
        }
    ...
    def update(self, update_size, rng):
        ...
        if self.l_type in LS_REAL:
            ...
            if self.next_layer.l_type in LS_REAL:
                upper_pot = jnp.dot(
                    (self.next_layer.values - self.next_layer.mean)
                ...
                upper_pot = (
                    jnp.dot((self.next_layer.values - self.next_layer.mean), fb_w)
                    / self.next_layer.temp
                )
    ...
    def record_trace(self, gate=None, lambda_=0):
        ...
        if gate is not None:
            v_ch = v_ch * gate[:, None]
        self.w_trace = (
            self.w_trace * lambda_ + self.inputs[:, :, None] * v_ch[:, None, :]
        )
    ...
    def learn_trace(self, reward, lr=0.01):
        ...
        self._w, self.opt_state_w = adam_update_jit(
            self._w, w_update, self.opt_state_w, lr
        )
        self._b, self.opt_state_b = adam_update_jit(
            self._b, b_update, self.opt_state_b, lr
        )
...
class Network:
    def __init__(
        self, state_n, action_n, hidden, var, temp, hidden_l_type, output_l_type
    ):
        self.layers = []
        in_size = state_n
    ...
    def map_grad_ascent(
        self, steps, state=None, gate=False, lambda_=0, update_size=0.01, rng=None
    ):
        # Thread the RNG key through all layer updates.
        for i in range(steps):
            for n, a in enumerate(
                self.layers if state is not None else self.layers[:-1]
            ):
            ...
            if i == steps - 1:
                gate_c = (
                    1 / (self.layers[-1].values - self.layers[-1].mean)[:, 0]
                    if gate
                    else None
                )
                for a in self.layers:
...
def main():
    ...
    for j in range(n_run):
        critic_net = Network(
            state_n=env.state.shape[1],
            action_n=1,
            hidden=hidden,
            var=critic_var,
            temp=None,
            hidden_l_type=critic_l_type,
        ...
        for i in range(int(1e9)):
            ...
            if value_old is not None:
                reward = (
                    jnp.clip(env.reward, -reward_lim, +reward_lim)
                    if reward_lim > 0
                    else env.reward
                )
                targ_value = reward + gamma * mean_value_new * (~env.isEnd).astype(
                    float
                )
                ...
                cur_critic_lr = linear_interpolat(
                    start=critic_lr_st, end=critic_lr_end, end_t=end_t, cur_t=i
                )
                cur_actor_lr = linear_interpolat(
                    start=actor_lr_st, end=actor_lr_end, end_t=end_t, cur_t=i
                )
            ...
            rng = critic_net.map_grad_ascent(
                steps=map_grad_ascent_steps,
                state=None,
                gate=True,
                lambda_=critic_lambda_,
                update_size=critic_update_size,
                rng=rng,
            )
            actor_net.clear_trace(~prev_isEnd)
            rng = actor_net.map_grad_ascent(
                steps=map_grad_ascent_steps,
                state=None,
                gate=False,
                lambda_=actor_lambda_,
            ...
            prev_isEnd = env.isEnd
            state, reward, isEnd, info = env.step(
                from_one_hot(action) if dis_act else action
            )
            c_eps_ret += reward
            if jnp.any(isEnd):
                eps_ret_hist.extend(c_eps_ret[isEnd].tolist())
                c_eps_ret = c_eps_ret.at[isEnd].set(0.0)
            if len(eps_ret_hist) >= max_eps:
                break
            if i * batch_size > print_count and len(eps_ret_hist) > 0:
                eps_ret_hist_jp = jnp.array(eps_ret_hist)
                f_str = (
                    "Run %d: Step %d Eps %d\t Running Avg. Return %f\t Max Return %f \t"
                )
                f_arg = [
                    j + 1,
                    i,
                    len(eps_ret_hist),
                    jnp.average(eps_ret_hist_jp[-100:]),
                    jnp.amax(eps_ret_hist_jp),
                ]
...
class BatchEnvs:
    def __init__(self, name, batch_size=1, rest_n=100, warm_n=100):
        ...
        obs, full_state = jax.vmap(self.env.reset, in_axes=(0, None))(
    ...
    def step(self, actions):
        ...
        step_fn = lambda key, st, action: self.env.step(
            key, st, action, self.env_params
        )
