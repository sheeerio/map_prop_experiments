...
hidden = json.loads(
    config.get("USER", "hidden")
)  # Number of hidden units on each layer
critic_l_type = config.getint(
    "USER", "critic_l_type"
)  # Activation function for hidden units in critic network; 0 for softplus and 1 for ReLu
actor_l_type = config.getint(
    "USER", "actor_l_type"
)  # Activation function for hidden units in actor network; 0 for softplus and 1 for ReLu
temp = config.getfloat("USER", "temp")  # Temperature for actor network if applicable

critic_var = json.loads(
    config.get("USER", "critic_var")
)  # Variance in the normal distribution of critic network's layer
critic_update_adj = config.getfloat(
    "USER", "critic_update_adj"
)  # Step size for minimizing the energy of critic network equals to the layer's variance multiplied by this constant
critic_lambda_ = config.getfloat(
    "USER", "critic_lambda_"
)  # Trace decay rate for critic network

actor_var = json.loads(
    config.get("USER", "actor_var")
)  # Variance in the normal distribution of actor network's layer
actor_update_adj = config.getfloat(
    "USER", "actor_update_adj"
)  # Step size for minimizing the energy of actor network equals to the layer's variance multiplied by this constant
actor_lambda_ = config.getfloat(
    "USER", "actor_lambda_"
)  # Trace decay rate for actor network

map_grad_ascent_steps = config.getint(
    "USER", "map_grad_ascent_steps"
)  # number of step for minimizing the energy
reward_lim = config.getfloat("USER", "reward_lim")  # whether limit the size of reward

critic_lr_st = json.loads(
    config.get("USER", "critic_lr_st")
)  # Learning rate for each critic network's layer at the beginning
critic_lr_end = json.loads(
    config.get("USER", "critic_lr_end")
)  # Learning rate for each critic network's layer at the beginning
actor_lr_st = json.loads(
    config.get("USER", "actor_lr_st")
)  # Learning rate for each actor network's layer at the end
actor_lr_end = json.loads(
    config.get("USER", "actor_lr_end")
)  # Learning rate for each actor network's layer at the end
end_t = config.getint(
    "USER", "end_t"
)  # Number of step to reach the final learning rate (linear interpolation for in-between steps)
...
for j in range(n_run):
    ...
    for i in range(int(1e9)):
        ...
        if value_old is not None:
            if reward_lim > 0:
                reward = np.clip(reward, -reward_lim, +reward_lim)
        ...
        if i * batch_size > print_count and len(eps_ret_hist) > 0:
            ...
            f_arg = [
                ...
                i,
                len(eps_ret_hist),
                np.average(eps_ret_hist[-100:]),
                np.amax(eps_ret_hist),
            ]
